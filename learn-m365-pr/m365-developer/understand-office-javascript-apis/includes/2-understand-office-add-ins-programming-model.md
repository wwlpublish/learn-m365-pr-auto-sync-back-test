...
You may not always have control over the version of Office your users have installed. To handle this situation, we provide requirement sets to help you determine whether an Office host supports the capabilities you need in your Office Add-in.
...
Along with adding this CDN link, all Office Add-ins require an `Office.onReady()` call. It's an asynchronous method that returns a Promise object while it checks to see if the Office.js library is loaded. When the library is loaded, it resolves the Promise as an object that specifies the Office host application with an `Office.HostType` enum value (for example, Excel, Word) and the platform with an `Office.PlatformType` enum value (PC, Mac, Office on the web). The Promise resolves immediately if the library is already loaded when `Office.onReady()` is called.
...
The `sync()` API call in Office.js returns a promised proxy object that is resolved when the host application completes the requested read or write actions. Multiple actions are batched up into a single proxy object that is synced to the host, rather than sending separate requests for each action.
