...
1. Server-side code uses the OAuth2 "on behalf of" flow to obtain an access token for Microsoft Graph in exchange for the bootstrap access token.
...
If the user has consented to the requested Microsoft Graph permissions, your add-in will receive the bootstrap token that you can use to exchange with Microsoft Graph using the OBO OAuth2 flow to obtain an access token that can be used to call Microsoft Graph.
...
If need more control over the request to Microsoft Graph, including calling a specific endpoint or providing more control over the URL parameters, you can use the `getGraphData()` method:
