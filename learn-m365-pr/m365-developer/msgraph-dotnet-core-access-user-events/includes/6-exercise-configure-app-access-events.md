...
9.	Save *GraphCalendarClient.cs* before continuing.

10.	Open *Pages/Calendar.cshtml.cs* and take a moment to explore the existing code. Note the following features:
...
11.	Locate the `OnGetAsync` method and replace the existing code with the following code:
...
1.	Once the page loads, a `GET` request is made to the Microsoft Graph `/me/calendarView` endpoint and the access token is passed in the Authorization Header. The call to `/me/calendarView ` will then retrieve the data securely from the service and display it in the page.
...
You have successfully demonstrated how to access and display Microsoft 365 calendar events for a signed in user using Microsoft Graph and ASP.NET Core! 
