...
To acquire phone numbers and phone service from Microsoft, you need to have one or more users with the Teams Phone license and one or more users with the Calling plan license.

If you don't already have them, to purchase a Teams Phone add-on license and Calling Plan license, perform the following steps:
...
1. Back in the Microsoft 365 admin center, perform the steps for all licenses still missing in your tenant, such as the Teams Phone add-on.
...
1. Select the **City** you want from the list.
...
1. Under Area code, select a numeric area code.

1. Select **Next**, a set of numbers will be reserved for you for 10 minutes. If you're happy with the numbers press place order to add them to your tenant. If you take more than 10 minutes, the phone numbers will be returned to the pool of numbers.

1. If you're happy with the numbers offered, Select **Place Order**

1. you'll see, thank you, your order has been placed. Select **Finish**

The new numbers will appear in the list of phone numbers as soon as they become available.
...
You can port or transfer phone numbers in all the supported countries/regions. Most types of numbers can be ported in, but not all. See the link in references for more information on what can and can't be ported.
...
1. Select a **Country/region.** Some regions will only support porting in service numbers. Review the reference links for a link to Microsoft Learn.
...
1. Fill in the Authorizing person on account details, that is, the details of the person on the existing providers account; **organization name, name, job title, phone number, email address, organization address**.

1. Fill in the **current provider details**, **the Billing Telephone Number (BTN), Account number, carrier name, and address**.

1. Fill in the **service address**. This address is what the existing operator would associate with the customer and number range.

1. Upload the number range in a CSV Comma-Separated Value) file that has only one column named PhoneNumber (note, one word, no spaces). Each phone number is listed per row in E.164 format, for example, +1425550199
...
After performing the described steps, you've completed a port request using the porting wizard.
...
1. This link will take you to the **Phone Number Service Center**.
...
After performing the described steps, you've performed a manual number port from another operator to Microsoft.

## Create request to convert service number to user number or user number to service number

Sometimes you may need to covert user numbers to service numbers or service numbers to user numbers. You can make this conversion by contacting the Microsoft PSTN Service Desk.
...
1. Microsoft will make the change and you'll receive an update in the portal under **view my existing cases.**

After performing the described steps, you've converted Service number to User number or User number to service number.
