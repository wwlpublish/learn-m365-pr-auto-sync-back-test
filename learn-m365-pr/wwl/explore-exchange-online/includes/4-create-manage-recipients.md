...
:::row:::
  ...
  :::column-end:::
  :::column:::
    Commonly used to create a global company distribution list. The list uses attributes in the company address book, such as region, office, department, or roles.
  :::column-end:::
:::row-end:::
:::row:::
  :::column:::
...
Directory synchronization is implemented in a Microsoft 365 tenant when either Azure AD Connect is installed. Azure AD Connect synchronizes the local Active Directory to Azure Active Directory. When an organization implements Azure AD Connect for synchronization, all recipient management tasks must be completed locally. In doing so, the on-premises Exchange management tools such as the Exchange Admin Center or Exchange Management Shell must be used to create, modify, and delete recipients.

## **Exercise – Interactive demonstrations**

The following simulations guide you through the steps to complete various recipient management tasks in Exchange Online. You'll use the Exchange admin center to manage recipients, create and manage Microsoft 365 groups, upgrade distribution lists, configure a group naming policy, and create a resource mailbox.
