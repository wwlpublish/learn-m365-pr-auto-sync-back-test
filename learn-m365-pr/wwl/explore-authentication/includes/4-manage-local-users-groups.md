...
For example, Elyssa works for a company that has a policy where users do not have administrative privileges to their computer. In thier position, however, they needs to frequently change the network settings on here device. As a standard user, they would not be able to do this. By making them a member of the Network Configuration Operators group, they now have the ability to change their network settings, without IT having to grant them full administrative privileges to the device. If their account or device is compromised, their credentials cannot be used to perform malicious attacks such as logging onto the device remotely.
