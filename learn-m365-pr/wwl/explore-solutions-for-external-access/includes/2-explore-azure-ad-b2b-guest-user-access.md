...
With Azure AD B2B, the external partner uses its own identity management solution. This design provides the following benefits to your organization:

 *  Azure AD isn't required.
 *  There's no external administrative overhead required by your organization.
    
     *  You don't have to manage external accounts or passwords.
...
Guest users sign in to your apps and services with their own work, school, or social identities. If the guest user doesn't have a Microsoft account or an Azure AD account, one is created for them when they redeem their invitation. The process for inviting and redeeming a guest user includes:
...
As an administrator, you can easily add guest users to your organization in the Azure portal. The process to add a guest user includes:

 *  Create a new guest user in Azure AD, similar to how you add a new user.
...
**Additional reading.** For more information, see [Azure Active Directory B2B collaboration licensing guidance](https://docs.microsoft.com/azure/active-directory/b2b/licensing-guidance?azure-portal=true).
