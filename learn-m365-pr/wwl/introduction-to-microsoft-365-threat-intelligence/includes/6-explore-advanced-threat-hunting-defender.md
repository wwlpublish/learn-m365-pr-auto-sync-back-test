...
The syntax of the tabular expression statement has tabular data flow from one tabular query operator to another. The flow starts with the data source (for example, a table in a database, or an operator that produces data) and then flows through a set of data transformation operators. These operators are bound together by using the pipe (\|) delimiter.
...
:::image type="content" source="../media/kusto-query-example-2-1b734494.png" alt-text="Screenshot of a basic Kusto query that's a tabular expression statement.":::
...
:::image type="content" source="../media/kusto-example-with-bad-file-endings-a576357e.png" alt-text="Screenshot of a Kusto query for process creation with suspicious file ending.":::
