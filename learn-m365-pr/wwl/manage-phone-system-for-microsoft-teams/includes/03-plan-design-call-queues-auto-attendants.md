...
- Do you want to enable [call parking and retrieval](/microsoftteams/call-park-and-retrieve?azure-portal=true) in your organization to help with call handoffs between people or departments?
...
Calls redirected out of a queue can be sent to any of the call routing destinations previously listed, except for an operator. Call queues don't have operators, but you can redirect callers to the same destination as an operator that you've configured for an auto attendant.
