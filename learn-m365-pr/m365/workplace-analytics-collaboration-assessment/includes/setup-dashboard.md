The Collaboration assessment dashboard uses a prebuilt Power BI template populated by output (OData) from two Workplace Analytics queries, Standard meeting and Collaboration assessment.

![Workflow of Collaboration assessment dashboard with queries](../media/collaboration-assessment-queries.png)

To set up the Collaboration assessment dashboard:

1. Run the Collaboration assessment and Standard meeting queries.
2. Download the Collaboration assessment Power BI template and load the query output.
3. Select your settings in the Collaboration assessment dashboard.
...
**1. Run the Collaboration assessment and Standard meeting queries**. The Collaboration assessment dashboard reports require output from two Workplace Analytics queries. On the Queries page, under **Start from preselected filters and metrics**, begin by running the Collaboration assessment and Standard meeting queries.

In the upper right corner of the Collaboration Assessment query, you'll notice a Power BI icon. This indicates there's a download option for a Power BI template that you can use to generate the reports in the dashboard.
...
- In the Collaboration assessment query, under **Select Organizational attributes**, you can leave the default attributes (Organization, LevelDesignation) selected, or select any other attributes to filter by. Be sure to select the attribute that identifies people supervisors in your company (if you cannot find a supervisor indicator field in the drop-down menu, contact your Workplace Analytics Admin to confirm it was included in the organizational data). 
...
**2. Download the Collaboration assessment PBI template and load the query output**. Once the queries have finished running, you can access the Collaboration assessment Power BI template and data through the OData URL by clicking the **Results** tab on the Queries page. 

![Download the template and load the query](../media/load-data-template.png)

To load the data into the Collaboration assessment Power BI (PBI) template:

1. Download the PBI template from the Collaboration assessment result.
1. When prompted to paste the OData link directly in the PBI file, click **“OK**.
1. Open the downloaded PBI template.
1. Paste the Collaboration assessment OData link into the Collaboration assessment Query URL field.
...
1. Enter the name of the supervisor indicator field you selected when running the Collaboration Assessment query.
...
**3. Select your settings in the Collaboration assessment dashboard**. After you've loaded the data and the report is generated, the dashboard opens to the Settings page, where you have the option to adjust some settings for your analysis.
...
Once you've selected the settings for the population you would like to analyze, you can start using the dashboard. The populated Collaboration assessment dashboard includes report pages on total collaboration, after hours, meeting culture, multitasking behaviors, manager relationships and more. 

For a more detailed list of step-by-step instructions on Collaboration assessment dashboard setup and troubleshooting see the link below under **Learn more**. There you'll also find template updates, setup instructions with video, things to remember when sharing the dashboard, FAQs, errors and troubleshooting, and support. 

## Learn more

[Collaboration assessment dashboard](https://docs.microsoft.com/Workplace-Analytics/tutorials/power-bi-collab-assess?azure-portal=true)
