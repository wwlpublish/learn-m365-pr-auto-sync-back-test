...
AppLocker helps reduce administrative overhead and the organization's cost of managing computing resources by decreasing the number of help desk calls that result from users running unapproved applications. By using AppLocker, you can:
...
AppLocker includes default rules for each rule collection. These rules help ensure that the files required for Windows to operate properly are allowed in an AppLocker rule collection. The default rules block many scripts, Windows installer packages, and executables. The default rules also allow members of the local **Administrators** group to run all Windows Installer files. The default rules are:
<!--LM: The following bullets shouldn't use bold formatting but you should still use bold formatting for the group and folder names.-->
