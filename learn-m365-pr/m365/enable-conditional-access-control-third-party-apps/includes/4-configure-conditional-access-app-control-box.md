Box provides Contoso's users with cloud-based storage and collaborative tools and apps. However, you want to be sure that users are not downloading, or indeed uploading, files that might contain malicious content. You can use Defender for Cloud Apps to help manage these requirements by creating Conditional Access App Control session or access policies.

The process for configuring Conditional Access App Control with Defender for Cloud Apps for Box is broadly similar to the process we reviewed in the last unit for Workplace from Facebook.
...
The final stage is to create the Access control or Session control policies. In this example, we'll create an access policy. In the **Defender for Cloud Apps** portal, complete the following steps:
