...
You start by creating a session policy in Defender for Cloud Apps. This session policy enables you to control user sessions. In effect, the user session is redirected through a reverse proxy instead of connecting directly to the targeted app. This arrangement means that all user access is directed through Defender for Cloud Apps, providing you with the controls discussed earlier.
