You can use Defender for Cloud Apps log collector to help automate log upload from your corporate network. The Defender for Cloud Apps log collector automatically processes, compresses, and transmits each log to the Defender for Cloud Apps portal for your review.

## Overview of traffic logs

Cloud Discovery uses the data in your traffic logs. The more information that is stored in your logs, the better understanding of your organizational apps. The following list describes the required attributes for log data ingested by Defender for Cloud Apps:
...
For Syslog files, the log collector writes the received logs to the disk. When the file size is larger than 40 KB, the log collector uploads the file to Defender for Cloud Apps. With FTP, the log collector uploads the logs to Defender for Cloud Apps as soon as the log collector receives the completed FTP upload.

After a log is uploaded, Defender for Cloud Apps moves the log to a backup directory.
...
After you've deployed the log collector, you must define it in the Defender for Cloud Apps portal:
...
Having completed the configuration procedure for Defender for Cloud Apps, you must now configure the log collector itself. As mentioned earlier, you can deploy in two modes: as a container, or a virtual appliance.
...
6. Next, import the configuration information from the Next steps section of the Create log collector dialog box.
   ...
   > [!TIP]
   > You can verify successful deployment on the Log collectors tab in the Defender for Cloud Apps portal. If your log collector shows as **Connected**, you have successfully completed deployment.
