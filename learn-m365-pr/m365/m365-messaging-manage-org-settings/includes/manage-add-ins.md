...
1. Extend to a larger set of individuals within the business who will be using the add-in. Again, evaluate results and, if all went well, go to the next step of a full deployment.
1. Deploy to target audience of users.
...
1. Select **Deploy Add-in** at the top of the page. On the overview page, select **Next**.
1. Select an option and follow the instructions.
1. If you're adding an add-in from the Office Store, make your add-in selection. Notice that you can sort available add-ins by **Suggested for you**, **Rating**, or **Name**. You can only add free add-ins from the Office Store. Paid add-ins aren't currently supported. After you've selected your add-in, review and accept any additional terms and conditions.
    >[!NOTE]
...
1. On the next page, select **Everyone**, **Specific users/groups**, or **Just me** to specify who the add-in is deployed to. You can search for users or groups.

    >[!NOTE]
    >Learn about the other states that apply to an add-in. See **Add-in states** in the **Learn more** section below.

1. Select **Deploy**.
1. A green tick appears when the add-in has been deployed. You can follow the on-page instructions to test that the add-in has deployed successfully.
    >[!NOTE]
...
1. When the add-ins are all deployed, select **Next**. If you've deployed to just yourself, select **Change who has access to add-in** to deploy to more users.
...
1. In the Microsoft 365 admin center, go to **Settings > Add-ins**.
1. Select the add-in you want to turn on or off.
1. Toggle the status to turn the add-in On or Off.
1. Save the changes.

### Edit add-in access

You can modify who can access the add-ins.

1. In the admin center, go to **Settings > Services & add-ins**.
1. Select the add-in you want to edit.
1. Click **Edit** under **Who has Access**.
1. Save the changes.
...
1. In the admin center, go to **Settings > Services & add-ins**.
1. Select **User owned apps and services**.
1. Clear the option to let users access the Office store.

### Delete an add-in

You can also delete an add-in.

1. In the admin center, go to **Settings > Services & add-ins**.
1. Select the add-in you want to delete.
1. Click **Delete Add-In**. Remove the Add-in button on the bottom right corner.
1. Validate your selections, and choose **Remove add-in**.
...
1. Go to the **Admin Managed** tab in the Office Add-ins window.
