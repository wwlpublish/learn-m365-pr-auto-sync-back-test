You’ve learned about MSIX packages. Now let's see how you can deploy those applications into your Windows Virtual Desktop environment and determine if MSIX app attach is right for Contoso.
...
- You don't need additional infrastructure servers to deploy the MSIX packages. You can use existing Azure Files for hosting the **vhd** that contains the MSIX package.
...
When MSIX app attach is launched, the application files are accessed from a virtual hard disk (VHD), and the user isn’t even aware that the application isn’t locally installed.
...
> **MSIX app attach** can be used on-premises, Azure, or other clouds environments without any modification and is independent of the OS.
