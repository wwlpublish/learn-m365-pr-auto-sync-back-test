Workplace Analytics provides you with four primary ways to analyze and explore your data.

- The **Home** page displays the scope of your Workplace Analytics data and allows you to navigate to high-level insights aligned to three research-based solution areas.
- The **Analyze** page contains two valuable tools: Explore dashboards and flexible queries. Dashboards let you quickly compare base metrics by group, while queries allow you to customize row-level data for more in-depth analysis.
- The **Plans** page works with MyAnalytics, where available, to provide targeted programs to improve focus, collaboration, well-being, and seller success.
- The **Settings** page allows you to validate collaboration data, organizational data, and optional CRM data. It also lets you define what calendar activity, if any, to exclude from your analysis.
...
In addition to the Explore dashboards, Workplace Analytics offers views into three areas of the collaborative experience:
...
### Analyze > Explore > Metrics overview (dashboards) page

The Explore dashboards provide a quick way to look at Workplace Analytics metrics summarized by group or over time. While the Home page provides a handful of leader-ready insights related to employee experience, organizational agility, and customer focus, the Explore dashboards cover a wider range of metrics for analysis, and can be useful to quickly identify patterns you want to investigate further.
...
The Explore dashboards reflect a rolling window of the six most recent months of weekly collaboration data.
...
**Queries** give you access to row-level data and customizable metrics. You can import the row-level data into analysis tools to create custom analyses, models, and dashboards.

On the Queries page, under **Start custom query**, you’ll see a list of query options. The two most common are:
...
You’ll see that three of the options shown above have a Power BI icon in the upper right corner. These include a download option for a Power BI Template. If you run this query option, you can download the template, load your data, and get an automatically generated report.

## Workplace Analytics Plans page

Insights gained from the Explore dashboards and queries can provide a lot of information about how employees in your organization get work done. With Workplace Analytics Plans, you can transform these insights into actions.
...
Under **Settings**, you will find the **Data sources** page. There you can get a quick view of collaboration data for your measured population over time. You can use this view to identify any dates or date ranges you should include or exclude from your analysis, such as holidays.
...
Kevin, our HR business analyst at Contoso Corporation, would like to get a high-level look at some fourth quarter custom metrics using a flexible query. He knows he should probably exclude specific dates that might be outliers. 

Kevin opens the **Sources** page and looks at the collaboration data over time for the measured population. He sees two dips in the data, both national holidays. He runs his flexible query, and once he has the output, he filters out the holiday dates for his analysis.
...
Workplace Analytics has a built-in solution to address these issues called **meeting exclusions**. With meeting exclusions, analysts can create specific rules for meetings to exclude while adding each rule to a dropdown list of rules for specific analyses.
...
- Click the **Bell** icon for notifications and alerts related to recent activity in Workplace Analytics. This feature shows dates of data refresh, organizational data uploads, newly created meeting exclusions, and query auto-refresh. Notifications alert you when your queries have finished running.
- Click the **?** (Help) icon to find:
