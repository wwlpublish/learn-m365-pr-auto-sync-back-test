...
Incidents ensure elements otherwise spread across various portals and queues are presented in a single coherent view, which improves your security team’s ability to manage threats and attacks.

Grouping related alerts into an incident helps you see:
...
In the illustration, an attacker starts with a spear-phishing email targeting a specific user. The email contains a malicious link to download a file that contains the Meterpreter payload. With the malicious code running on the target device, the attackers perform reconnaissance to understand which users have signed into the device and which other devices these users have access to.

In this scenario, the attacker finds the credentials of an IT helpdesk team member. Impersonating this IT helpdesk team member via overpass-the-hash, the attackers are able to move laterally to a second device. On a new device, they steal the user’s web credentials, which they use to remotely access the user’s cloud apps like OneDrive or SharePoint. This allows the attackers to insert a malicious macro into an existing online Word document, which they then deploy in a lateral phishing attack by distributing links to the malicious document to other users in the organization.

Microsoft 365 Defender can automatically collect and correlate isolated alerts and other related security events into a single incident, reducing workloads so security analysts have fewer, more comprehensive work items in their incident queues. These incident queues bring together related alerts, affected assets, and other evidence, making it easier to understand the complete attack story and take informed actions.
...
You’ll use the incident queue to sort through incidents, and to prioritize and create an informed cybersecurity response decision. Microsoft 365 Defender allows you to view incidents from many different viewpoints:
...
- Extended list of filters, which improves your ability to analyze incidents using more types of filters including investigation state, device groups, and OS platforms.
- Full alignment with the Microsoft Defender for Endpoint alert queue.

## Prioritizing incidents

Microsoft 365 Defender applies correlation analytics, aggregates related alerts, and automated investigations from different products into an incident. Microsoft 365 Defender also triggers unique alerts on activities that can only be identified as malicious given the end-to-end visibility that Microsoft 365 Defender has across the entire suite of products.

### Sorting and filters

From the default incident queue, you can select **Filters** to see a Filters pane, from which you can view a filtered set of incidents.

:::image type="content" source="../media/2-incidents-ss-incidents-filters.png" alt-text="Screenshot showing the use of Filters in the incident queue":::

There are around a dozen different filters available. Some of the more useful filters are:

- **Classification** – you’ll use this filter to display incidents that are classified as either true or false alerts, or those that are waiting to be classified.
- **Severity** – this lets you get an indicative indicator of the impact of the incident, with a high number requiring immediate attention.

## Managing incidents

Incident management is critical in ensuring that threats are contained and addressed. There are several ways you can manage your incidents:
...
:::image type="content" source="../media/2-incidents-manage.png" alt-text="Screen shot showing how to manage incidents":::

### Incident name

Microsoft 365 Defender automatically assigns a name based on alert attributes such as the number of endpoints affected, users affected, detection sources or categories. This allows you to quickly understand the scope of the incident.
...
1. For each incident, begin an attack and alert analysis:
   1. View the summary of the incident to understand its scope and severity and what entities are affected (the Summary tab).
   1. Begin analyzing the alerts to understand their origin, scope, and severity (the Alerts tab).
   1. As needed, gather information on affected devices, users, and mailboxes (the Devices, Users, and Mailboxes tabs).
   1. See how Microsoft 365 Defender has automatically resolved some alerts (the Investigations tab).
   1. As needed, use the data set for the incident for more information (the Evidence and Response tab).
...
1. Resolve the incident and take time for post-incident learning to:
   1. Understand the type of the attack and its impact.
   1. Research the attack in Threat Analytics and the security community for a security attack trend.
   1. Recall the workflow you used to resolve the incident and update your standard workflows, processes, policies, and playbooks as needed.
   1. Determine whether changes in your security configuration are needed and implement them.
