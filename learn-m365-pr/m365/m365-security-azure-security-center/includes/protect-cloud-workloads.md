...
Here, you'll learn how you can use Azure Defender capabilities to protect these different workloads.
...
1. In Azure Security Center, select **Just-in-time VM** access on the left pane, under **Advanced cloud defense**.
...
1. Select the **Recommended** tab. You'll see groups of machines for which Azure Security Center recommends you apply application controls to define a list of known-safe applications.
...
1. Go to the **Recommendations** pane in Azure Security Center.
...
1. You can choose between different options. Select **Recommended: Deploy ASC integrated vulnerability scanner powered by Qualys (included in standard pricing tier)**.
...
When it comes to containers, Azure Security Center provides protection for the following resource types:

| **Resource type**                              | **Details**                                                  |
| ---------------------------------------------- | ------------------------------------------------------------ |
| **Container   hosts**                          | Azure   Security Center will scan your Docker configuration and inform you of   misconfigurations through a list of failed rules that were assessed. |
| **Azure   Kubernetes Service (AKS) clusters**  | Through the Azure Security Center's AKS bundle, you can gain better insight into   the security state of your nodes, cloud traffic, and more. |
| **Azure   Container Registry (ACR registries** | Through   the Azure Security Center's ACR bundle, you'll gain deeper information   about vulnerabilities of images in your ACR registries. |

> [!NOTE]
> Use the links in the **Learn more** section of this to find out more about the AKS and ACR bundles.

Azure Security Center provides protection for your containers and their apps through several measures, including:
...
To use vulnerability management, you'll need to make sure you're on the Azure Security Center standard tier. You can then enable the Container Registries bundle. You do this by going to **the Pricing & settings** pane, and then enabling **Container registries**:

:::image type="content" source="../media/4-enable-container-registries.png" alt-text="Enable container registries":::

When an image is pushed to your registry, Azure Security Center will scan it using a Qualys scanner. If issues are identified, you'll receive recommendations through the Azure Security Center's **Recommendations** pane. Select a
...
Azure Security Center continuously monitors your Docker configurations and Azure Kubernetes clusters. For example, it will identify unmanaged containers hosted on Linux virtual machines that run Docker containers. Azure Security Center will assess the configurations of your containers continuously, and compare them against the Center for Internet Security (CIS) Docker Benchmark.

Azure Security Center will then tell you if there are any containers that don't meet a particular control. After it gives you recommendations, you can implement them as follows:

1. Select **Compute & Apps** in the left pane of Azure Security Center, under **Resource security hygiene**.
...
- **AKS cluster level**: When you enable the **Kubernetes option** from the **Pricing & settings** pane, Kubernetes audit logs are analyzed through agentless monitoring. Azure Security Center then monitors AKS-managed services using logs fetched by AKS. It will monitor for events like the creation of sensitive mounts or any high privileged roles.

Alerts will show up in Azure Security Center when a suspicious event occurs. These alerts help you to remediate security issues on time, and improve the protection of your containers.

## Data and storage protection

Azure Security Center helps you protect your data and storage. Azure Security Center will detect threats to your data and storage resources, alert you, and suggest remediation steps to deal with those threats. With Azure Security Center, you don't have to be an expert, and you don't need to learn multiple security monitoring systems to keep your data and storage secure. Azure Security Center helps you protect different data and storage resources and services including:
...
Azure Security Center provides specific protections for each resource or service. For example:
...
1. Go to the **Pricing & settings** pane in Azure Security Center, then select your subscription.
...
For example, Azure Security Center will monitor for activities across your Azure Storage accounts and alert you based on the type of event that has occurred. You'll receive alerts for:
...
1. Go to the **Pricing & settings** pane in Azure Security Center, and select your subscription.
1. Make sure you're on the standard tier of Azure Security Center.
