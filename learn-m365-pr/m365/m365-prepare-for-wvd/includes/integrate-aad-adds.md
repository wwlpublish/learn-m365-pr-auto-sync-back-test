...
You can use the [Microsoft Azure Active Directory Connect tool](https://www.microsoft.com/download/details.aspx?id=47594) to set up synchronization.

## Architecture and connection options for Active Directory Domain Services 
In Windows Virtual Desktop, your remote sessions use Active Directory Domain Services (AD DS) the same way that your current virtual and physical desktop environment on premises does for session logins at the VM layer. You have the following options to connect with or provision AD DS for Windows Virtual Desktop:  
